...
function init(root) {
  var kua = {
    ...
    option: {},
    pathify: function pathify(it) {
      return Module.globalPaths.push(it);
    },
    exec: function exec(it) {
      return childProcess.execSync(it).toString();
    },
    spawn: function spawn(command) {
    ...
    color: function color(c, v) {
      return kua.config.color && '\u001b[38;5;' + c + 'm' + v + '\u001b[0m' || v;
    },
    glob: glob.sync,
    uuid: uuid
  };
  ...
  var log = kua.option.daemon && fs.openSync('kua.log', 'a+');
  ...
  if (fs.existsSync(root + '/config.yml')) {
    (0, _deepExtend2.default)(kua.config, yaml.load(fs.readFileSync(root + '/config.yml', 'utf8')));
  }
  if (fs.existsSync(root + '/host.yml')) {
    (0, _deepExtend2.default)(kua.config, yaml.load(fs.readFileSync(root + '/host.yml', 'utf8')));
  }
  (0, _deepExtend2.default)(this, kua);
  return this;
}
